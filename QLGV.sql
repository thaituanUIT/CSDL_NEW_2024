...
GO
--CAU I.19-----------------------------------------------
CREATE TRIGGER TRG_INS_LUONGGV
ON GIAOVIEN
...
BEGIN
    IF EXISTS (
        ...
        ON gv.HOCVI = ins.HOCVI
           AND gv.HOCHAM = ins.HOCHAM
           AND gv.HESO = ins.HESO
           AND gv.MUCLUONG <> ins.MUCLUONG
    )
    BEGIN
        RAISERROR ('LOI: MUC LUONG KHONG DONG NHAT', 16, 1)
        ROLLBACK TRANSACTION;
    END
    ELSE
    BEGIN
...
BEGIN
    IF EXISTS (
        ...
        JOIN KETQUATHI AS kt
        ON ins.MAHV = kt.MAHV
           AND ins.MAMH = kt.MAMH
           AND ins.LANTHI = kt.LANTHI + 1
        WHERE kt.DIEM >= 5
    )
    BEGIN
        RAISERROR ('DIEM THI LAN 1 CUA HOC VIEN KHONG DUOI 5 DIEM', 16, 1)
        ROLLBACK TRANSACTION;
    END
    ELSE
    BEGIN
...
GO
--CAU I.23-----------------------------------------------
CREATE TRIGGER TRG_INS_DKMONHOC
ON GIANGDAY
INSTEAD OF INSERT, UPDATE
AS
BEGIN
    IF EXISTS (
        ...
        WHERE NOT EXISTS (
            ...
            WHERE kq.MAHV IN (
                SELECT MAHV
                FROM HOCVIEN
                WHERE HOCVIEN.MALOP = ins.MALOP
            )
            AND kq.MAMH = dk.MAMH_TRUOC
            AND kq.KQUA = 'Đạt')      
    )
    BEGIN
        RAISERROR ('LOI: MON HOC CHUA DU DIEU KIEN', 16, 1);
        ROLLBACK TRANSACTION;
    END
    ELSE
    BEGIN
        PRINT ('PHAN CONG MON HOC THANH CONG')
    END
END
